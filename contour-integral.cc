...
int main(void)
{
  // verification program for the contour integral

  // the theorem is available when |a|,|b|<|q^(-1/2)| and |q|<1
  // reference
  // HYPERBOLIC BETA INTEGRALS
  // Jasper V. Stokman, 2004, arXiv
...
cout.precision(16);
 ...
 if(abs(A)>=abs(pow(q,-1/2))||abs(B)>=abs(pow(q,-1/2)))
   {
     cout<<"this program is unavailable" <<endl;
   }
 else
   {
     //calculate (qa,qb;q)_Infinity
     // first, (qa;q)_Infinity
     int j=1;
