...
// q-exponential function e_q(z), E_q(z)
// q-sin, q-cos
...
template <class T> interval<T> infinite_qPochhammer(const interval<T>& z,const interval<T>& q){
  ...
  */
  ...
  template <class T> interval<T> q_Exp(const interval<T>& z,const interval<T>& q){
   ...
   template <class T> interval<T> q_Sinh(const interval<T> & z,const interval<T>& q){
    // verification program for q-sinh function Sinh_q(z)
    complex<interval<T> >res;
    res=(q_Exp(interval<T> (z),interval<T>(q))-q_Exp(interval<T> (-z),interval<T>(q)))*0.5;
    return res;
  }
